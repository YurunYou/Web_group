...
from models.user_model import get_all_users, unreturn_user
import datetime
...
def admin_staff():
    ...
    ids=unreturn_user()
    print(ids)
    return render_template('admin/staff/admin_staff.html', users = users, ids =ids )
...
def admin_delete_staff(id):
    delete_staff( id)
    flash(f'staff has been deleted successsfully', category='success')
    return redirect(url_for('admin_module.admin_staff'))
